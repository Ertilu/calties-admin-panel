...
const OutForms = () => {
  ...
  const _submit = useCallback(async () => {
    let result
    setLoading(true)
    result = await OutStockService.create({
      ...data,
  ...
  const isButtonDisabled = useMemo(
    () => data?.inventoryId === '' || data?.out === '' || data?.vendor === '' || loading,
    [data, loading],
  )
  ...
  return (
    ...
    >
      ...
      <CContainer>
        <CCard>
          <CCardHeader>
            <strong>Tambah Data Keluar</strong>
          </CCardHeader>
          <CCardBody>
            <CForm>
